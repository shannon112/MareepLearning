...
for n in [2]:
    ...
    # fusion
    #y_pred = (y_pred_1 + (y_pred_2*y_mean_1/y_mean_2))/2    
    y_pred = (1.75*y_pred_1*y_mean_2/y_mean_1 + y_pred_2)/2
...
y_KMeans = MiniBatchKMeans(n_clusters=2, random_state=0).fit(y_projected)
...
print(y_KMeans.cluster_centers_[1])
...
plt.legend()
plt.title("pca")
plt.savefig("img/fcn_pca_result.png")

# output result
with open('submission/prediction_fcn_pca.csv', 'w') as f:
    f.write('id,anomaly\n')
    for i in range(len(y_pred)):
        f.write('{},{}\n'.format(i+1, y_pred[i]))
"""
